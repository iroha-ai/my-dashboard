...
作成日: 2026-08-07
最終更新: 2026-08-08（「これからの一週間」右隣りのGoogleカレンダー埋め込みを定時ニュースダイジェストのリンク一覧に置き換え〈要gmail.readonly再同意・動作未確認〉。週間天気アイコン+テキスト化。運行情報のジョルダン検知動作確認・鮮度閾値修正）
...
**⚠️ 動作未確認:** `gmail.readonly`はスコープ追加のため、既存の同意には含まれていない。
**次回「接続する」を押したときに、Googleの同意画面が変わる（または再表示される）はず**
だが、このセッションでは実機確認していない。うまく一覧が出ない場合は、まず
「接続する」を押し直してGmailへのアクセスを許可できているか確認すること。
...
- [x] 今日の予定（広め、テキスト一覧）・これからの一週間（右隣りに定時ニュース
      ダイジェストのリンク一覧。コードは実装済みだが実機動作未確認、下記「未完了・
      次にやること」参照）
...
### 4. 定時ニュース欄：Gmail API有効化・スコープ再同意・動作確認が未実施（優先度高）

上記「『これからの一週間』の右隣り：Googleカレンダー埋め込み→定時ニュースへ置き換え」
参照。コードは実装済みで`main`にプッシュ済みだが、**このセッションでは実機確認して
いない。** 次回開くときに確認すること:

1. Google Cloud コンソールで **Gmail API** が有効化されているか（README「1. Google
   カレンダー・タスク・Gmailに繋ぐ」参照。他のAPIと同様、有効化直後は見た目が
   紛らわしいので、ボタンの状態まで確認する）
2. ブラウザで「接続する」を押し直し、`gmail.readonly`まで含めて許可できるか
   （スコープを追加したので、既存の同意には含まれていない）
3. 「これからの一週間」の右隣りの「定時ニュース」欄に、実際のメール一覧
   （新しい順）が表示されるか。表示されない場合はブラウザの開発者ツールで
   コンソールのエラー（`メール検索に失敗 (403)`等）を確認する
4. リンクをクリックして、Gmail上で正しいメールが開くか
