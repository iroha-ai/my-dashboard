...
作成日: 2026-08-07
最終更新: 2026-08-07（Google Tasks連携・OAuthスコープ拡張後）
...
- タスク（来客・外出の右隣）: Google Tasks API（`@default`リストの未完了分、期限順）。予定と同じトークンを使い回す
...
- 「会議」欄を廃止した。来客・外出のピックアップのみ残す
- 来客・外出と今日の予定を横並びに変更。今日の予定の欄を広め（`grid-template-columns: 1fr 2.4fr`）にした
- 左サイドバー（昭島市週間天気）の下に「運行情報」（中央線・青梅線・銀座線）を追加。
  当初はライブ取得を実装したが、下記の理由でやめて公式ページへのリンクに切り替えた
- 「タスク」欄を来客・外出と今日の予定の間に追加（`grid-template-columns: 1fr 1fr 2.2fr`）。
  Google Tasks APIから取得。詳細は下記「タスク連携」参照
- 来客・外出／タスクを「一週間分」から「今日ぶんだけ」の表示に変更した。
  一週間分の見え方は「これからの一週間」欄で確認できるので重複を減らす狙い
...
（2026-08-07、来客・外出も含めてこの欄は「今日ぶんだけ」に統一した。期限なし・
今日以外のタスクは出ない）。
...
- OAuthスコープに`tasks.readonly`を追加（既存の`calendar.readonly`との併記）。
  スコープを広げると既存の同意では足りなくなるため、変更後にHideが一度「接続する」を押し直している
...
- [x] 左サイド：昭島市 週間天気（7日）
- [x] 来客・外出ピックアップ（今日の予定と横並び）
- [x] タスク（Google Tasks・来客の右隣・未完了を期限順）
- [x] 今日の予定（広め）・これからの一週間
...
[モニター PC ブラウザ]
   ...
   ├→ タスク: Google Tasks API（予定と同じトークン・リポジトリ非経由）
   └→ 運行情報: 静的リンク（取得なし）
```
...
- 「来客・外出と今日の予定の横幅比率を変えて」（Hide の指示があれば）
