...
作成日: 2026-08-07
最終更新: 2026-08-08（.centerを3列×3行の単一グリッドに統合し2/3行目の高さ比・列位置を調整。雨雲レーダー〈Yahoo!静止画地図API〉追加。今日の予定はテキスト一覧に戻し、Googleカレンダー埋め込みは「これからの一週間」の右隣りへ移動。週間天気アイコン+テキスト化。運行情報のジョルダン検知動作確認・鮮度閾値修正）
...
│ 昭島市   │  天気：昭島｜台東｜鹿沼                       │
│ 週間天気 ├──────┬──────┬────────────────┤  相場         │
│雨雲レーダ│来客・│タスク│ 今日の予定（広め） │ （widget×6）  │
│ 鹿沼市   │外出  │      │                  │               │
│ 週間天気 ├──────┴──────┴──────┬─────────┤  チャートへ   │
│雨雲レーダ│これからの一週間 │カレンダー│               │
│ 運行情報 │                │（Google  │               │
│（リンク  │                │ 埋め込み）│               │
│ ＋色分け）│                │          │               │
└──────────┴────────────────┴─────────┴───────────────┘
...
「これからの一週間」の右隣りにはGoogleカレンダーの公式埋め込み（週表示、閲覧専用）を
2026-08-08に追加した（詳細は下記「カレンダー埋め込み：『これからの一週間』の右隣りに設置」参照）。
左サイドの週間天気の下には、同日にYahoo!の雨雲レーダー画像を追加した
（詳細は下記「雨雲レーダー（Yahoo!静止画地図API）を追加」参照）。

**行の高さ比・列位置（2026-08-08調整）:** `.center`は3列×3行の単一グリッド
（`grid-template-columns: 1fr 1fr 2.2fr`）になっていて、2行目（来客・外出／タスク／
今日の予定）と3行目（これからの一週間／カレンダー）が同じ列トラックを共有している。
これにより「これからの一週間」の右端は「タスク」の右端に、「カレンダー」の左端は
「今日の予定」の左端にピクセル単位で揃う。行の高さは`grid-template-rows: auto
minmax(0, 0.85fr) minmax(0, 1.35fr)`（2行目を低く・3行目を広げる方向にHideの要望で調整。
以前は`1.2fr` / `1fr`だった）。
...
- `js/train.js`: `train.json` を読み、`.train-link[data-line="chuo|ome|ginza"]` の
  ...
  （Hideの要望どおり、色だけ）
- 取得できなかった・データが古い（90分以上更新なし。詳細は下記参照）ときは、
  **色を変えずそのまま**にする（「取れない＝平常運転」と決めつけない、という
  既存の設計方針を踏襲）

## 「運行情報の更新が止まっている可能性」の閾値を20分→90分に変更（2026-08-08）

**きっかけ:** 中央線・青梅線のジョルダン切替の動作確認中、銀座線側の`train.json`が
1時間40分以上更新されていないことに気づいた（`updatedAt`が`23:57:07Z`のまま停滞）。

**原因（実測で確認）:** `train.yml`は`cron: '*/5 * * * *'`（5分おき）で設定しているが、
実際の実行履歴の間隔を並べると `34分・36分・46分・47分・48分・51分・58分・59分・67分・69分・85分`
——**設定は5分おきでも、実際にはGitHub側の都合で30〜85分間隔でしか発火していなかった**
（ワークフロー自体は`active`、詰まっている実行もゼロ。GitHub公式ドキュメントにも
高負荷時にscheduleイベントが遅延する旨の記載があり、特に高頻度cronほど間引かれやすいと
されている。今回に限った不具合ではなく元からの仕様的な制約）。

**対応:** `js/train.js`の`STALE_MS`を、実測の最悪ケース（85分）に余裕を持たせて
**90分**に緩めた。20分のままだと、正常に動いていてもほぼ常に「止まっている可能性」の
誤警告が出る状態だった。

**この変更で解決しないこと:** cronの発火自体が実測30〜85分とばらつくという制約は
変わらない。90分に緩めたのは「誤警告を減らす」対応であって、「本当に長時間（90分超）
止まった場合の検知」までタイムリーにする対応ではない（そもそもの発火間隔が不安定なので、
これ以上厳しい閾値にすると誤警告が増えるだけになる）。より確実にしたい場合は、
GitHub Actionsのschedule機能に頼らず、外部の確実なスケジューラ（cron-job.orgなど）から
`workflow_dispatch`や`repository_dispatch`を叩く方式への切替が選択肢になるが、
「追加インフラを持たない」という要件定義の方針と相談が必要（未着手）。
...
- `gas/train-status-watcher.gs`: Google Apps Script。ジョルダンの「運行情報メール」を
  ...
  `updatedAt` が5分おきに更新され続け、フロント側の鮮度判定（90分ルール。
  下記「『運行情報の更新が止まっている可能性』の閾値を20分→90分に変更」参照）を
...
## カレンダー埋め込み：「これからの一週間」の右隣りに設置（2026-08-08）

**きっかけ:** Hideから「今日の予定欄にGoogleカレンダーの1日単位を埋め込み、日付も
切り替えられるようにしたい。修正（編集）もできると尚良い」との要望。実装後、
「今日の予定は元に戻す。これからの一週間の右隣りに新規にグーグルカレンダーを
埋め込む」という指示があり、**「今日の予定」欄は元のテキスト一覧に戻し**、
埋め込み自体は場所を移して残した。以下は現在の実装内容（移設後）。

**技術的な制約（先に確認したこと）:** Googleの認証済みインタラクティブなカレンダー
画面（`calendar.google.com/calendar/u/0/r/day` 等、編集ができる本物の画面）は、
Google側がクリックジャッキング対策として`X-Frame-Options: SAMEORIGIN`でiframe埋め込み
自体をブロックしている。実ブラウザUAで再確認済みで、回避不可（Google側の意図的な制限）。
一方、**公式の「埋め込みカレンダー」機能**（`calendar.google.com/calendar/embed`）は
iframe埋め込みが可能（こちらは`X-Frame-Options`が付かない）だが、**閲覧専用**。
この制約をHideに提示し、閲覧専用（日付切替えは可）の方向で了承済み。

**プライバシー面の配慮:** 埋め込みの`src`パラメータには実際のカレンダーID
（primaryカレンダーの場合、Hideのメールアドレスそのもの）が必要になる。これを
`js/config.js`に直書きすると、**公開リポジトリのソースにメールアドレスが残ってしまう**
（このプロジェクトが一貫して守ってきた「予定・タスクの中身をリポジトリに残さない」
方針に反する）。そのため、**既存のOAuthトークン（`calendar.readonly`、スコープ変更不要）で
`GET /calendar/v3/calendars/primary`を呼び、実行時にブラウザ内だけでカレンダーIDを
取得して埋め込みURLを組み立てる**実装にした。リポジトリにメールアドレスは一切残らない。

**実装（現在の姿）:**

- `index.html`: `.panel.week`（これからの一週間）と、新設した`.panel.week-calendar`
  （`#week-calendar-embed`という`<iframe>`＋`#week-calendar-placeholder`）を
  `.week-row`というグリッド（左右2カラム、`.center`内の3行目）で横並びにしている。
  `today`欄（`#today-list`）は元のテキスト一覧のみに戻し、埋め込みは持たない
- `js/calendar.js`: `ensureWeekCalendarEmbed(token)`関数（旧`ensureTodayEmbed`から
  改名・移設）。`calendars/primary`を叩いて取得したIDで埋め込みURL
  （`mode=WEEK`・`ctz=Asia/Tokyo`・タイトル/印刷/タブ/カレンダー一覧などの
  余計なUIは`show*=0`で消す）を組み立て、`iframe.src`にセットしたら`is-hidden`を外し、
  代わりに`#week-calendar-placeholder`（「読み込み中」表示）に`is-hidden`を足す
- 一度成功したら`weekEmbedInitialized`フラグでスキップする（毎回作り直さない。
  埋め込み自体はGoogle側のiframe内で完結し続けるため、こちらから触る必要が無い）
- **フォールバック:** `calendars/primary`の取得に失敗した場合（オフライン等）は、
  黙って「読み込み中」のプレースホルダーのままにする（今日の予定側と違い、
  代わりに表示するテキスト一覧を持たない。次回ポーリングで自動的に再試行される）。
  デモモード（`?demo=1`）では埋め込みを使わない

**副次的な効果（気づき）:** 埋め込みiframe自体はGoogleの認証Cookie（ブラウザの
Googleログインセッション）で動いているので、こちらのOAuthトークンの状態（上記
「Googleサインインが『毎回クリックが必要』な件」のサイレント再取得失敗など）に
左右されずに表示され続ける可能性がある。実機での長期挙動は未確認。

**編集について:** 上記の技術的制約により、埋め込み内での編集はできない。
編集したい場合は埋め込み内に案内は出ないので、本物のGoogle Calendarアプリを
別タブで開いてもらうしかない（今回はその案内リンクの追加までは行っていない。
必要なら追加できる）。

## 雨雲レーダー（Yahoo!静止画地図API）を追加（2026-08-08）

**きっかけ:** Hideから「週間天気の下に、昭島・鹿沼それぞれの雨雲レーダー図を貼り付ける
（ヤフーから）」との要望。

**技術的な検討:** Yahoo!天気・災害の雨雲レーダーページ（`weather.yahoo.co.jp`）は
実ブラウザUAでも`X-Frame-Options: SAMEORIGIN`が付いており、Google Calendarの
インタラクティブ画面と同様にiframe埋め込みができない。代わりに、**Yahoo!静止画地図API**
（YOLP = Yahoo! Open Local Platform の一部）が、`overlay=type:rainfall`パラメータで
現在時刻の雨雲を重ねた地図を**単なる画像として**返してくれる。これはただの`<img src="...">`
なのでiframeの制限は関係なく、素の静的サイトでも問題なく使える。

**実装:**

- `js/config.js`: `yahooMapAppId`を追加（空文字が初期値）
- `js/weather.js`: `renderRadar(city)`関数を追加。`updateWeeklyCity`から、週間天気の
  取得成否に関わらず呼ぶ（緯度経度とAPIキーだけで完結するため）。`yahooMapAppId`が
  空の間は「Yahoo!のappidが未設定」という案内文を出すだけにする
- 画像URL: `https://map.yahooapis.jp/map/V1/static?appid=...&lat=...&lon=...&z=10` +
  `&width=272&height=220&overlay=type:rainfall&style=base:monotone`（緯度経度は
  `weeklyWeatherCities`＝週間天気と同じ設定を流用）。キャッシュされないよう
  毎回`_`にタイムスタンプを足している
- `index.html`: `#akishima-radar` / `#kanuma-radar` を各都市の週間天気リストの直後に追加
- `css/styles.css`: `.rain-radar-img`（画像）・`.rain-radar-note`（未設定時の案内文）を追加

**⚠️ これもコードだけでは動かない。Hide側の登録が必要:**

1. <https://e.developer.yahoo.co.jp/register/> でYahoo!デベロッパーネットワークに
   無料登録し、アプリケーションを作成する（1日5万リクエストまでの制限あり）
2. 発行されたClient IDを `js/config.js` の `yahooMapAppId` に設定する

登録が済むまでは、雨雲レーダーの位置に「Yahoo!のappidが未設定」という案内文が
表示され続ける（エラー扱いにはしていない）。**この登録作業とAPIキーの動作確認は
このセッションでは未実施。** Hideが登録・設定したら、画像が正しく表示されるか
（appidが無効だとYahoo側がエラー画像やHTTPエラーを返す可能性がある）確認が必要。
...
- [x] 今日の予定（広め、テキスト一覧）・これからの一週間（右隣りにGoogleカレンダー公式埋め込み・日付切替え可・閲覧専用）
...
- [ ] 雨雲レーダー（週間天気の下、昭島・鹿沼）：コード実装済みだが、
      Yahoo!デベロッパーネットワークへの登録（`yahooMapAppId`設定）が未実施
      （下記「未完了・次にやること」4番）
...
### 4. 雨雲レーダーのAPIキー登録が未実施（優先度中・Hide側の作業待ち）

上記「雨雲レーダー（Yahoo!静止画地図API）を追加」参照。コードは実装済みで、
`js/config.js`の`yahooMapAppId`が空の間は案内文だけを表示する安全側の挙動に
なっている。Hideが<https://e.developer.yahoo.co.jp/register/>で登録し、
Client IDを設定したら、実際に画像が正しく表示されるか（appidの形式・
リクエスト制限など）の確認が必要。
