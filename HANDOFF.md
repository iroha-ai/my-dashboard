...
作成日: 2026-08-07
最終更新: 2026-08-08（今日の予定にGoogleカレンダーの公式埋め込みを追加。週間天気アイコン化。運行情報のジョルダン検知動作確認・鮮度閾値修正）
...
## 今日の予定：Googleカレンダーの公式埋め込みを追加（2026-08-08）

**きっかけ:** Hideから「今日の予定欄にGoogleカレンダーの1日単位を埋め込み、日付も
切り替えられるようにしたい。修正（編集）もできると尚良い」との要望。
...
- `index.html`: `#today-calendar-embed`という`<iframe>`を`#today-list`の上に追加
  （`class="is-hidden"`で初期状態は非表示）
- `js/calendar.js`: `ensureTodayEmbed(token)`関数を追加。`calendars/primary`を叩いて
  取得したIDで埋め込みURL（`mode=DAY`・`ctz=Asia/Tokyo`・タイトル/印刷/タブ/カレンダー
  一覧などの余計なUIは`show*=0`で消す）を組み立て、`iframe.src`にセットしたら
  `is-hidden`を外し、代わりに`#today-list`（従来のテキスト一覧）に`is-hidden`を足す
- 一度成功したら`embedInitialized`フラグでスキップする（毎回作り直さない。
  埋め込み自体はGoogle側のiframe内で完結し続けるため、こちらから触る必要が無い）
- **フォールバック:** `calendars/primary`の取得に失敗した場合（オフライン等）は、
  黙って従来の`#today-list`（API取得したイベントのテキスト一覧）のままにする。
  デモモード（`?demo=1`）でも同様に埋め込みは使わず、従来のテキスト一覧のまま
...
- [x] 今日の予定（広め、Googleカレンダー公式埋め込み・日付切替え可・閲覧専用）・これからの一週間
