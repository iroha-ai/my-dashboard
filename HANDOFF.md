...
## 「これからの一週間」の右隣り：Googleカレンダー埋め込み→定時ニュースへ置き換え（2026-08-08）

**経緯（3段階で変わった）:**

1. Hideから「今日の予定欄にGoogleカレンダーの1日単位を埋め込み、日付も切り替えられる
   ようにしたい。修正（編集）もできると尚良い」との要望 → 実装（詳細は下記「技術的な制約」）
2. 「今日の予定は元に戻す。これからの一週間の右隣りに新規にグーグルカレンダーを埋め込む」
   → 「今日の予定」欄は元のテキスト一覧に戻し、埋め込みは「これからの一週間」の右隣りへ移設
...
できる本物の画面）は、Google側がクリックジャッキング対策として`X-Frame-Options:
SAMEORIGIN`でiframe埋め込み自体をブロックしている（実ブラウザUAで確認済み、回避不可）。
公式の「埋め込みカレンダー」機能は埋め込み可能だが閲覧専用。この制約が結果的に、
3で「編集できないなら別のものに置き換えたい」という判断につながった可能性がある。

**「定時ニュースダイジェスト」とは:** Hideが別途運用している定時ニュース配信の
仕組みが、`youtree.3tree.3tree@gmail.com`から自分自身へ、1日に数回、件名
「定時ニュースダイジェスト YYYY-MM-DD HH:MM（日本時間）」でメールを送っている
（複数カテゴリのニュース記事とリンクをまとめたHTMLメール。関連:
`ops/methodology/定時ニュース配信では送信済みニュースを除外する.md`、AGENTS.md
「Scheduled News Delivery」節）。テスト送信は件名に「【テスト】」が付く。

**実装:**

- `js/news.js`（新規）: Gmail APIで`subject:(定時ニュースダイジェスト) -テスト`を
  検索し、該当メールの件名・日時だけを取得（本文は使わない）。**メール1通＝1行**、
  クリックするとGmail Web版でそのメールが開く（`https://mail.google.com/mail/u/0/#all/<messageId>`）。
  Gmailの検索結果は新しい順に返るため、そのまま並べれば「新しいものを上にアペンド」になる
- `js/calendar.js`: OAuthスコープに`gmail.readonly`を追加（後述）。`updateCalendar`の
  成功パスから`updateNewsDigest(token, onStatus)`を呼ぶ（旧`ensureWeekCalendarEmbed`を
  削除して置き換え）
- `index.html`/`css/styles.css`: `#week-calendar-embed`（iframe）を撤去し、
  `#news-digest-list`という`<ul>`に置き換え。パネルの見出しも「カレンダー」→「定時ニュース」。
  グリッド上の位置（`.week-calendar`クラス、今日の予定と同じ列）はそのまま流用
- `js/demo-events.js`: `DEMO_NEWS()`を追加。`?demo=1`では実IDが無くリンクを張れないため、
  テキストのみのダミー行を表示する

**⚠️ OAuthスコープについて（当初の想定と変わった点）:** 本当に必要なのは件名・日時
だけなので、最初は最小権限の`gmail.metadata`スコープを使うつもりだった。しかし
**`gmail.metadata`スコープは`messages.list`の`q`パラメータ（検索クエリ）に対応しておらず**、
「件名に定時ニュースダイジェストを含むメールだけ」をサーバー側で絞り込めないことが
分かった（全メール一覧を取得してクライアント側で絞り込むのは非現実的）。そのため、
本文まで読める広いスコープの**`gmail.readonly`**を使っている（実際に読むのは件名・
日時のみ）。この点はHideに確認していない。運用上問題があれば、`gmail.metadata`＋
`labelIds`によるラベルフィルタ（`q`を使わない別の絞り込み手段。要事前のラベル運用）
などへの変更を検討する。

**✅ 2026-08-08、実機確認済み。** 途中2つハマったが、いずれも解決した:

1. **`popup_failed_to_open`エラー**（コンソールに出た）: 定時ニュース機能自体の不具合
   ではなく、既存の「Googleサインインが毎回クリックが必要」な件（下記参照）そのもの
   だった。トークン期限切れ後の無言更新がポップアップブロックで失敗し、カレンダー・
   タスク・定時ニュース全部が更新に失敗する形で表面化した。「接続する」を押し直せば解消
2. **`メール検索に失敗 (403)`エラー**: Google Cloud ConsoleでGmail APIが未有効化
   だったのが原因（Tasks APIのときと同じパターン）。有効化して解決

**表示の調整（2026-08-08）:** 当初は日時のみの短い表示だったが、Hideの指定で
「日時＋件名＋ニュース件数」を1行に並べる形に変更した。件数は`format=metadata`では
取れない（ヘッダーに件数の情報が無い）ため、`format=full`で本文（HTML）も取得し、
**本文中の「原文を読む」等のボタンリンクの出現数を数えて件数の目安にする**という
ヒューリスティックにしている（`js/news.js`の`countArticles`）。メールのHTMLテンプレートに
強く依存しており、テンプレートが変わると数え損なう可能性があるが、その場合は件数を
表示しないだけで、日時・件名の表示・リンク自体は壊れない設計にしてある。
掲載件数もHideの指定で20件→10件に変更した。
...
- [x] 今日の予定（広め、テキスト一覧）・これからの一週間（右隣りに定時ニュース
      ダイジェストのリンク一覧。日時+件名+件数、最大10件、実機確認済み）
...
### 4. ~~定時ニュース欄：Gmail API有効化・スコープ再同意・動作確認~~（2026-08-08 解決済み）

上記「『これからの一週間』の右隣り：Googleカレンダー埋め込み→定時ニュースへ置き換え」の
「✅ 実機確認済み」参照。Gmail API有効化・`gmail.readonly`再同意・表示確認まで完了した。
