...
automation-2が登録キーワード・登録サイトを巡回して作る**定時ニュース見出し**を並べている。
JSONは毎日7:00・12:00・16:00に`data`ブランチの`news-headlines.json`へ差し替えられ、
ダッシュボードは見出しをクリックすると配信元記事または公式発表を直接開く。
２０２６年８月２３日まではGmailのダイジェストメールを中継していたが、現在はYahoo!ニュース
見出しと同じ公開JSON経路へ統一しており、ニュース表示にGoogle認証は使わない。
...
5. 画面右上の「接続する」を押し、`calendar.readonly`と`tasks.readonly`の権限を許可する
