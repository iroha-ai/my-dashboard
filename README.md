...
「今日の予定」欄は従来どおりAPI取得のテキスト一覧。**「これからの一週間」の右隣り**には、
**定時ニュースダイジェスト（Gmail）へのリンク一覧**を並べている（2026-08-08〜）。
Hideが別途運用している定時ニュース配信の仕組みが、自分宛に「定時ニュースダイジェスト
YYYY-MM-DD HH:MM（日本時間）」という件名でメールを送ってくるので、それをGmail API
で検索し、**メール1通＝1行**、クリックするとGmail上でそのメールが開くリンクとして
並べている（新しいものが上）。以前はここにGoogleカレンダーの週表示を埋め込んでいたが、
編集ができない（Google側の技術的な制約。詳細はHANDOFF.md参照）こともあり、
定時ニュースに置き換えた。
...
### 1. Google カレンダー・タスク・Gmailに繋ぐ

1. Google Cloud コンソールでプロジェクトを作り、**Google Calendar API**・
   **Google Tasks API**・**Gmail API** の3つを有効にする
...
5. 画面右上の「接続する」を押し、`calendar.readonly`・`tasks.readonly`・
   `gmail.readonly` の3つの権限を許可する
...
今日の予定／一週間／定時ニュースの欄がすべて「カレンダーに接続できていません」の
ままになる。
...
HANDOFF.md参照）。**Gmail APIも同様に、有効化できたか見た目で確認すること。**

**⚠️ 2026-08-08にGmail APIとgmail.readonlyスコープを追加したが、このセッションでは
実機での動作確認（Cloud ConsoleでのGmail API有効化、既存ユーザーの再同意フロー）は
できていない。** 定時ニュース欄が「定時ニュースの取得に失敗」のままなら、
まずGmail APIが有効化されているか、「接続する」を押し直してGmailへのアクセスまで
許可できているかを確認すること。
...
- **予定・タスク・メールの中身はリポジトリに残さない。** 取得結果をファイルに書き出さない
- **取れなかったものは黙って空にしない。** 天気・カレンダー・タスク・定時ニュースのいずれかが
  落ちたら、画面右上に短く出す
- **タスクの取得失敗は、画面上にHTTPステータスとGoogle側のエラーメッセージまで出す。**
  コンソールを開かなくても原因（権限不足・API未有効化など）を判断できるようにするため
- **`gmail.readonly`スコープは、本来必要な範囲（件名・日時）より広い。** `gmail.metadata`
  （最小権限）では検索クエリが使えなかったための妥協。実際に読むのは定時ニュースダイジェスト
  のメールの件名・日時だけで、本文やそれ以外のメールを読んだり画面に出したりはしない
  （`js/news.js`参照）
