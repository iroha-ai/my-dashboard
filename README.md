...
「今日の予定」欄は、**Googleカレンダーの公式埋め込み（1日表示、日付の切替え可）**を
そのまま表示している（2026-08-08〜）。ただし**閲覧専用**——Googleが認証済みの
インタラクティブなカレンダー画面自体はクリックジャッキング対策でiframe埋め込みを
ブロックしているため、公式に埋め込める「閲覧専用ウィジェット」の方を採用した。
編集したいときは、別タブでGoogle Calendarを開く必要がある。埋め込みURLに必要な
カレンダーID（実質メールアドレス）はリポジトリに書かず、既存のOAuthトークンで
実行時に取得している（詳細はHANDOFF.md「今日の予定：Googleカレンダーの公式埋め込みを
追加」参照）。埋め込みが読み込めなかった場合は、従来のテキスト一覧（API取得）に
自動でフォールバックする。
